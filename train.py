...
from model.transformer_xl import Transformer_XL

"""
ref : https://tutorials.pytorch.kr/beginner/torchtext_translation_tutorial.html
"""

model = Transformer_XL(vocab_size=vocab_size,
                       max_len=max_len,
                       m_len=m_len,
                       d_model=d_model,
                       d_head=d_head,
...
def train(model, iterator, optimizer, loss_fn):
    ...
    epoch_loss = 0
    ms = None
    for src, tgt in tqdm(iterator, desc='step'):
        optimizer.zero_grad()

        output, ms = model(src, ms)
